`timescale 1ns / 1ps

// ============================================================================
// TESTBENCH: tb_snn_spike_pattern
// DESCRIPTION:
//   Verifies deterministic per-neuron spike generation using an alternating
//   weight pattern across the 64-neuron parallel datapath.
//
// TEST CONFIGURATION:
//   - 64 physical neurons processed in parallel
//   - 4096 virtual neuron chunks
//   - 262,144 virtual neurons in total
//   - Even-indexed neurons receive +150
//   - Odd-indexed neurons receive +40
//   - Threshold = 100
//   - Leak factor = 1.0 (Q1.15 representation)
//   - Expected spike vector = 0x5555555555555555
//
// The test also verifies that the expected spike pattern remains stable across
// the complete virtual neuron address space and that TLAST is asserted on the
// final chunk.
// ============================================================================

module tb_snn_spike_pattern();

    // =========================================================================
    // TEST PARAMETERS
    // =========================================================================

    parameter PARALLEL_NEURONS = 64;
    parameter WEIGHT_W         = 16;
    parameter HEADROOM_BITS    = 10;
    parameter VMEM_W           = 32;
    parameter MAX_CHUNKS       = 4096;
    parameter LEAK_FRAC_BITS   = 15;
    parameter FIFO_DEPTH       = 8192;
    parameter ADDR_W           = 12;

    localparam WEIGHT_BUS_W = PARALLEL_NEURONS * WEIGHT_W;

    // =========================================================================
    // DUT INTERFACE SIGNALS
    // =========================================================================

    reg aclk;
    reg aresetn;

    reg ctrl_new_timestep;
    reg ctrl_clear_vmem;

    reg signed [VMEM_W-1:0] i_thresh;
    reg [15:0]              i_leak_factor;
    reg [ADDR_W-1:0]        i_max_chunks;

    // Spike input stream
    reg  s_axis_spike_tvalid;
    reg  s_axis_spike_tdata;
    reg  s_axis_spike_tlast;
    wire s_axis_spike_tready;

    // Weight input stream
    reg  s_axis_weight_tvalid;
    reg  [WEIGHT_BUS_W-1:0] s_axis_weight_tdata;
    wire s_axis_weight_tready;

    // Spike output stream
    wire [PARALLEL_NEURONS-1:0] m_axis_spike_tdata;
    wire                        m_axis_spike_tvalid;
    wire                        m_axis_spike_tlast;
    reg                         m_axis_spike_tready;

    // =========================================================================
    // DEVICE UNDER TEST
    // =========================================================================

    vrm_snn_core #(
        .PARALLEL_NEURONS(PARALLEL_NEURONS),
        .WEIGHT_W(WEIGHT_W),
        .HEADROOM_BITS(HEADROOM_BITS),
        .VMEM_W(VMEM_W),
        .MAX_CHUNKS(MAX_CHUNKS),
        .LEAK_FRAC_BITS(LEAK_FRAC_BITS),
        .FIFO_DEPTH(FIFO_DEPTH),
        .RAM_STYLE("ultra"),
        .ADDR_W(ADDR_W)
    ) DUT (
        .aclk(aclk),
        .aresetn(aresetn),

        .ctrl_new_timestep(ctrl_new_timestep),
        .ctrl_clear_vmem(ctrl_clear_vmem),

        .i_thresh(i_thresh),
        .i_leak_factor(i_leak_factor),
        .i_max_chunks(i_max_chunks),

        .s_axis_spike_tvalid(s_axis_spike_tvalid),
        .s_axis_spike_tdata(s_axis_spike_tdata),
        .s_axis_spike_tlast(s_axis_spike_tlast),
        .s_axis_spike_tready(s_axis_spike_tready),

        .s_axis_weight_tvalid(s_axis_weight_tvalid),
        .s_axis_weight_tdata(s_axis_weight_tdata),
        .s_axis_weight_tready(s_axis_weight_tready),

        .m_axis_spike_tdata(m_axis_spike_tdata),
        .m_axis_spike_tvalid(m_axis_spike_tvalid),
        .m_axis_spike_tlast(m_axis_spike_tlast),
        .m_axis_spike_tready(m_axis_spike_tready)
    );

    // =========================================================================
    // CLOCK GENERATOR
    // =========================================================================
    // 10 ns period = 100 MHz.

    initial begin
        aclk = 1'b0;
        forever #5 aclk = ~aclk;
    end

    // =========================================================================
    // INPUT DATA GENERATION
    // =========================================================================

    integer n;
    integer s;

    reg [WEIGHT_BUS_W-1:0] dummy_weights;

    // -------------------------------------------------------------------------
    // Push one input chunk.
    //
    // Every input spike is accompanied by a complete weight vector. Even
    // neuron indices receive weight_even, while odd neuron indices receive
    // weight_odd.
    // -------------------------------------------------------------------------

    task push_mixed_chunk(
        input integer num_spikes,
        input signed [15:0] weight_even,
        input signed [15:0] weight_odd
    );
        begin
            dummy_weights = {WEIGHT_BUS_W{1'b0}};

            for (n = 0; n < PARALLEL_NEURONS; n = n + 1) begin
                if ((n % 2) == 0)
                    dummy_weights[(n*WEIGHT_W) +: WEIGHT_W] = weight_even;
                else
                    dummy_weights[(n*WEIGHT_W) +: WEIGHT_W] = weight_odd;
            end

            for (s = 0; s < num_spikes; s = s + 1) begin
                s_axis_spike_tvalid  <= 1'b1;
                s_axis_spike_tdata   <= 1'b1;
                s_axis_spike_tlast   <= (s == num_spikes - 1);

                s_axis_weight_tvalid <= 1'b1;
                s_axis_weight_tdata  <= dummy_weights;

                // Wait for simultaneous acceptance of both input streams.
                do begin
                    @(posedge aclk);
                    #1;
                end while (!(s_axis_spike_tready &&
                             s_axis_weight_tready));
            end

            s_axis_spike_tvalid  <= 1'b0;
            s_axis_weight_tvalid <= 1'b0;
            s_axis_spike_tlast   <= 1'b0;
        end
    endtask

    // =========================================================================
    // OUTPUT COMPLETION DETECTION
    // =========================================================================

    reg spike_tlast_caught;
    reg clear_spike_flag;

    // Detect TLAST from the final output chunk.
    always @(posedge aclk) begin
        if (!aresetn || clear_spike_flag) begin
            spike_tlast_caught <= 1'b0;
        end
        else if (m_axis_spike_tvalid && m_axis_spike_tready) begin
            if (m_axis_spike_tlast) begin
                spike_tlast_caught <= 1'b1;

                $display(
                    "\n[OUTPUT] Final chunk received. "
                    "TLAST asserted after %0d chunks.",
                    MAX_CHUNKS
                );
            end
        end
    end

    // =========================================================================
    // OUTPUT MONITOR AND CHECKER
    // =========================================================================

    integer error_count;
    integer chunk_count_out;

    reg [PARALLEL_NEURONS-1:0] expected_vector;

    initial begin
        error_count    = 0;
        chunk_count_out = 0;
        expected_vector = {PARALLEL_NEURONS{1'b0}};
    end

    always @(posedge aclk) begin
        if (m_axis_spike_tvalid && m_axis_spike_tready) begin

            // Verify the complete output spike vector.
            if (m_axis_spike_tdata !== expected_vector) begin
                $display(
                    "[ERROR] Chunk %0d | Expected = %h | Got = %h",
                    chunk_count_out,
                    expected_vector,
                    m_axis_spike_tdata
                );

                error_count = error_count + 1;
            end

            // Limit terminal output for the large-scale test.
            if ((chunk_count_out < 3) ||
                (chunk_count_out >= MAX_CHUNKS - 3)) begin

                $display(
                    "[MONITOR] Chunk %0d | Output = %0h | "
                    "Expected = %0h | %s",
                    chunk_count_out,
                    m_axis_spike_tdata,
                    expected_vector,
                    (m_axis_spike_tdata == expected_vector)
                        ? "PASS"
                        : "FAIL"
                );
            end
            else if (chunk_count_out == 3) begin

                $display(
                    "[MONITOR] Output for intermediate chunks suppressed."
                );
            end

            chunk_count_out = chunk_count_out + 1;
        end
    end

    // =========================================================================
    // MAIN TEST SEQUENCE
    // =========================================================================

    integer c;

    initial begin
        $display("=========================================================");
        $display("SNN SPIKE PATTERN VERIFICATION");
        $display("=========================================================");
        $display("Configuration:");
        $display("  Physical neurons : %0d", PARALLEL_NEURONS);
        $display("  Virtual chunks   : %0d", MAX_CHUNKS);
        $display(
            "  Virtual neurons  : %0d",
            PARALLEL_NEURONS * MAX_CHUNKS
        );
        $display("  Threshold        : 100");
        $display("  Leak factor      : 1.0 (Q1.15)");
        $display("  Even neuron weight: +150");
        $display("  Odd neuron weight : +40");
        $display("  Expected pattern : 0x5555555555555555");
        $display("=========================================================");

        // ---------------------------------------------------------------------
        // Initial conditions
        // ---------------------------------------------------------------------

        aresetn = 1'b0;

        ctrl_new_timestep = 1'b0;
        ctrl_clear_vmem   = 1'b0;

        s_axis_spike_tvalid  = 1'b0;
        s_axis_spike_tdata   = 1'b0;
        s_axis_spike_tlast   = 1'b0;

        s_axis_weight_tvalid = 1'b0;
        s_axis_weight_tdata  = {WEIGHT_BUS_W{1'b0}};

        // Continuously drain the output FIFO.
        m_axis_spike_tready = 1'b1;

        clear_spike_flag = 1'b0;

        // ---------------------------------------------------------------------
        // SNN configuration
        // ---------------------------------------------------------------------

        i_thresh      = 32'sd100;
        i_leak_factor = 16'h7FFF;
        i_max_chunks  = MAX_CHUNKS - 1;

        // ---------------------------------------------------------------------
        // Reset release
        // ---------------------------------------------------------------------

        #100;
        aresetn = 1'b1;
        #100;

        // ---------------------------------------------------------------------
        // Timestep 1: Checkerboard spike pattern
        //
        // Even-indexed neurons:
        //   +150 >= 100 -> spike
        //
        // Odd-indexed neurons:
        //   +40 < 100 -> no spike
        //
        // Therefore the expected 64-bit output is:
        //   0101...0101 = 0x5555555555555555
        // ---------------------------------------------------------------------

        expected_vector = 64'h5555555555555555;
        chunk_count_out = 0;
        error_count     = 0;

        $display("\n---------------------------------------------------------");
        $display("TIMESTEP 1: CHECKERBOARD SPIKE PATTERN");
        $display("---------------------------------------------------------");
        $display("Even-indexed neurons : +150 -> SPIKE");
        $display("Odd-indexed neurons  :  +40 -> NO SPIKE");
        $display("Expected output      : 5555555555555555");
        $display("---------------------------------------------------------");

        // Clear virtual membrane memory and start a new timestep.
        @(posedge aclk);
        #1;

        ctrl_clear_vmem   = 1'b1;
        ctrl_new_timestep = 1'b1;

        @(posedge aclk);
        #1;

        ctrl_clear_vmem   = 1'b0;
        ctrl_new_timestep = 1'b0;

        // Process the complete virtual neuron address space.
        for (c = 0; c < MAX_CHUNKS; c = c + 1) begin
            push_mixed_chunk(
                1,
                16'sd150,
                16'sd40
            );
        end

        // Wait until the final chunk reaches the output interface.
        while (spike_tlast_caught == 1'b0) begin
            @(posedge aclk);
            #1;
        end

        clear_spike_flag = 1'b1;

        @(posedge aclk);
        #1;

        clear_spike_flag = 1'b0;

        // ---------------------------------------------------------------------
        // Final verification result
        // ---------------------------------------------------------------------

        $display("\n=========================================================");

        if ((error_count == 0) &&
            (chunk_count_out == MAX_CHUNKS)) begin

            $display("TEST PASSED");
            $display(
                "Checkerboard spike pattern remained stable across "
                "%0d chunks.",
                MAX_CHUNKS
            );
            $display(
                "Verified virtual neuron count: %0d.",
                PARALLEL_NEURONS * MAX_CHUNKS
            );
        end
        else begin

            $display("TEST FAILED");
            $display(
                "Detected %0d output errors across %0d chunks.",
                error_count,
                chunk_count_out
            );
        end

        $display("=========================================================");

        #200;
        $finish;
    end

endmodule
